...
class onnxruntime_subplugin final : public tensor_filter_subplugin
{
  ...
  const GstTensorFilterProperties *filter_props;
  ...
  OrtTensorRTProviderOptionsV2 * createTensorRtOptions (
    OrtApi api, const gchar *modelDir,
    gboolean isModelDirWriteable,
    gboolean isCudaGraph);
...
void
onnxruntime_subplugin::configureSession (bool force_fallback)
{
  g_assert_nonnull (filter_props);
  ...
  /* Read a model */
  try {
    if (force_fallback && fallbackSessionOptions) {
      session = Ort::Session (env, model_path, fallbackSessionOptions);
      fallbackSessionOptions = Ort::SessionOptions( nullptr );
    } else {
  ...
  auto tensors_info = const_cast<GstTensorsInfo *> (&filter_props->output_meta);
  ...
  if (prop && prop->custom_properties) {
    ...
    for (int i = 0; custom_properties[i]; i++) {
      if (strcmp(custom_properties[i], "disable_cuda") == 0) {
        ...
        disable_rocm = true;
      } else if (strcmp(custom_properties[i], "disable_openvino") == 0) {
...
onnxruntime_subplugin::createTensorRtOptions (
  OrtApi api, const gchar *modelDir,
  gboolean isModelDirWriteable,
  gboolean isCudaGraph)
{
 ...
 */
void
onnxruntime_subplugin::setAccelerator (const char *accelerators, bool invoke_dynamic)
{
  use_gpu = TRUE;
  ...
  if (has_tensorrt && (use_accelerator & ACCL_GPU) && enable_tensorrt) {
...

    {
      ...
      OrtTensorRTProviderOptionsV2* options = createTensorRtOptions(api, modelDir, isModelDirWriteable, true);
      OrtTensorRTProviderOptionsV2* fallbackOptions = createTensorRtOptions(api, modelDir, isModelDirWriteable, false);
      sessionOptions.AppendExecutionProvider_TensorRT_V2(*options);
      fallbackSessionOptions.AppendExecutionProvider_TensorRT_V2(*fallbackOptions);
      api.ReleaseTensorRTProviderOptions(options);
      api.ReleaseTensorRTProviderOptions(fallbackOptions);

      g_info("onnxruntime_subplugin::setAccelerator tensorrt");
      g_free(modelDir);
    }
    {
      ...
      std::vector<const char*> values{"1", "HEURISTIC", cudaStreamAsString.c_str()};
    ...
    }
    {
      ...
      api.ReleaseCUDAProviderOptions(options);
      g_info("onnxruntime_subplugin::setAccelerator tensorrt: set %ld CUDA options on fallbackSessionOptions", keys.size());
    }
    ...
    const GstTensorMemory *input, GstTensorMemory *output)
{
  ...
  try {
    invokeDynamic(prop, input, output);
  } catch (const Ort::Exception &exception) {
    if (fallbackSessionOptions) {
      g_info ("ONNX provider error '%s' for %s, trying to use fallback session options",
          exception.what (), model_path);
